...
public class Job {
	...
	public Job(String transcodedFile) {
		this.transcodedFile = transcodedFile;
	}

	public void runJob() {
		try {
			final JobGraph graph = new JobGraph("Live Streaming Demo");

			final JobInputVertex streamHandler = new JobInputVertex(
					"PacketizedStreamHandler", graph);
			streamHandler.setInputClass(PacketizedStreamHandler.class);
			streamHandler.setNumberOfSubtasks(1);
			streamHandler.getConfiguration().setInteger(
					PacketizedStreamHandler.STREAM_HANDLER_SERVER_PORT, 7711);
			...
			overlay.getConfiguration().setString(
					TwitterOverlayProvider.TWITTER_USERNAME_KEY,
					"danielwarneke");
			overlay.getConfiguration().setString(
					TwitterOverlayProvider.TWITTER_PASSWORD_KEY, "");
			overlay.getConfiguration().setString(
					TwitterOverlayProvider.TWITTER_KEYWORD_KEY, "google+");
			...
			output.getConfiguration().setString(
					...
					"file://" + this.transcodedFile);

			streamHandler.connectTo(decoder, ChannelType.INMEMORY);
			...
			graph.addJar(new Path(
					userHome
							+ "/.m2/repository/org/twitter4j/twitter4j-core/2.2.5/twitter4j-core-2.2.5.jar"));
			graph.addJar(new Path(
					userHome
							+ "/.m2/repository/org/twitter4j/twitter4j-stream/2.2.5/twitter4j-stream-2.2.5.jar"));
	...
	public static void main(final String[] args) {
		if (args.length != 1) {
			System.out.println("Usage: livestreamjob <transcoded-file>");
			System.exit(1);
		}

		String transcodedFile = args[0];

		Job job = new Job(transcodedFile);
		job.runJob();
	}
}
