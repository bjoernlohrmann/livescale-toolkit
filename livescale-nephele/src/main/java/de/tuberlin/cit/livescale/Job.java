...
public class Job {
	...
	public void runJob() {
		try {
			...
			final JobInputVertex streamHandler = new JobInputVertex("PacketizedStreamHandler", graph);
			streamHandler.setInputClass(PacketizedStreamHandler.class);
			streamHandler.setNumberOfSubtasks(1);
			streamHandler.getConfiguration().setInteger(PacketizedStreamHandler.STREAM_HANDLER_SERVER_PORT, 7711);
			...
			overlay.getConfiguration().setString(TwitterOverlayProvider.TWITTER_PASSWORD_KEY, "");
			overlay.getConfiguration().setString(TwitterOverlayProvider.TWITTER_KEYWORD_KEY, "google+");
			...
			output.setFilePath(new Path(transcodedFile));

			streamHandler.connectTo(decoder, ChannelType.INMEMORY, CompressionLevel.NO_COMPRESSION);
	...
	public static void main(final String[] args) {
		if (args.length != 1) {
			System.out
				.println("Usage: livestreamjob <transcoded-file>");
			System.exit(1);
		}
